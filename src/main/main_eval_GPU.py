...
def _evaluate_models(
    ...
    # x_batch [batch_size,3,244,224] y_batch [batch_size]
    # explain_data 3*(batch_size,XAI_methods,3,224,224)
    # print(x_batch.shape,y_batch.shape,len(explain_data),explain_data[0].shape)
    # print('oct Y',y_batch)
    for idx_model, model in tqdm(
        ...
        eval_data.append(torch.stack(eval_data_model,dim=0))
        del model
        torch.cuda.empty_cache()
        gc.collect()
    
    # 3*(Xai_methods,eval_metrics,batch_size)
    print(len(eval_data),eval_data[0].shape)
    return eval_data
...
def _evaluate_single_model(
    ...
    idx_model: int,
    explain_data: List[torch.Tensor],
    x_batch: torch.Tensor,
    y_batch: torch.Tensor,
) -> List[torch.Tensor]:
    ...
    for idx_xai in tqdm(
        ...
        eval_data_model.append(torch.stack(results,dim=0))
...
def _evaluate_chunks(
    ...
    for idx_chunk in tqdm(
        ...
        # for i in scores:
        #     print(i.shape)
        results.append(torch.stack(scores,dim=0))

        del eval_methods, scores
        torch.cuda.empty_cache()
        gc.collect()
...
def _get_saliency_chunk(
    ...
    if torch.all(a_batch[idx_chunk : idx_chunk + cfg.chunk_size] == 0):
...
def _save_evaluation_scores(
    cfg: DictConfig, datamodule: LightningDataModule, eval_data: List[torch.Tensor]
) -> None:
    ...
    torch.save(eval_data, file_name)

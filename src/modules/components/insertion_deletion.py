...
class BaseEvaluation(metaclass=ABCMeta):
    ...
    @abstractmethod
    def __call__(self, heatmap, *args, **kwargs) -> dict:
        raise NotImplementedError

class Insertion(BaseEvaluation):
    ...
    def __call__(self, model, x_batch, y_batch, a_batch, **kwargs):
        ...
        return np.array(insertion_auc_total)
...
class Deletion(BaseEvaluation):
    def __init__(self, pixel_batch_size=10, modality="image"):
        self.pixel_batch_size = pixel_batch_size
        self.modality = modality
    ...
    def __call__(self, model, x_batch, y_batch, a_batch, **kwargs):
        ...
        return np.array(deletion_auc_total)

...
from src.modules.components.eval_metric_GPU import (
    Insertion_GPU, Deletion_GPU, NegativePerturbation_GPU, PositivePerturbation_GPU, 
    FaithfulnessCorrelation_GPU, FaithfulnessEstimate_GPU, MonotonicityCorrelation_GPU, 
    IROF_GPU, RegionPerturbation_GPU, Infidelity_GPU)
...
def config(hparams, modality, **kwargs):
    ...
    metric = FaithfulnessCorrelation_GPU(
        nr_runs=hparams["nr_runs"],
        subset_size=hparams["subset_size"],
        similarity_func=hparams["similarity_func"],
        perturb_effect=hparams["perturb_effect"],
        perturbation=hparams["perturbation"]
    )
    return metric
...
def config(hparams, modality, **kwargs):
    ...
    metric = FaithfulnessEstimate_GPU(
        similarity_func=hparams["similarity_func"],
        features_in_step=hparams["features_in_step"],
        perturbation=hparams["perturbation"],
        perturb_effect=hparams["perturb_effect"]
...
def config(hparams, modality, **kwargs):
    ...
    metric = MonotonicityCorrelation_GPU(
        features_in_step=hparams["features_in_step"],
        perturbation=hparams["perturbation"],
        similarity_func=hparams["similarity_func"],
        perturb_effect=hparams["perturb_effect"]
...
def config(hparams, modality, **kwargs):
    ...
    #     modality=modality,
    # )
    metric = Insertion_GPU(
        pixel_batch_size=hparams["pixel_batch_size"],
        sigma=hparams["sigma"],
        kernel_size=hparams["kernel_size"],
        perturbation=hparams["perturbation"],
        preserve=hparams["preservation"],
        modality=modality,
    )
...
@eval_metric_registry.register_metric("negative")
def config(hparams, modality, **kwargs):
    metric = NegativePerturbation_GPU(
        pixel_batch_size=hparams["pixel_batch_size"],
        modality=modality,
        perturbation=hparams["perturbation"],
        preserve=hparams["preservation"]
...
@eval_metric_registry.register_metric("positive")
def config(hparams, modality, **kwargs):
    metric = PositivePerturbation_GPU(
        pixel_batch_size=hparams["pixel_batch_size"],
        modality=modality,
        perturbation=hparams["perturbation"],
        preserve=hparams["preservation"]
